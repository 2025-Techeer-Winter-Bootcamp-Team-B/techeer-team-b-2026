...
async def collect_rent_transactions(
    start_ym: str = Query(..., description="시작 연월 (YYYYMM)", min_length=6, max_length=6, examples=["202001"]),
    end_ym: str = Query(..., description="종료 연월 (YYYYMM)", min_length=6, max_length=6, examples=["202512"]),
    max_items: Optional[int] = Query(None, description="최대 수집 개수 제한 (None=제한 없음)", ge=1),
    allow_duplicate: int = Query(0, description="중복 저장 허용 여부 (0=건너뛰기, 1=업데이트)", ge=0, le=1),
    db: AsyncSession = Depends(get_db)
    ...
    Args:
        ...
        allow_duplicate: 중복 저장 허용 여부 (기본값: 0, 0=건너뛰기, 1=업데이트)
        db: 데이터베이스 세션
    ...
    try:
        ...
        logger.info(f"   🔄 중복 처리: {'업데이트' if allow_duplicate == 1 else '건너뛰기'}")
        logger.info("=" * 60)
