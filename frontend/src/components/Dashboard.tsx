...
import { getRecentViews, RecentView, deleteRecentView, deleteAllRecentViews } from '../lib/usersApi';
...
export default function Dashboard({ onApartmentClick, onRegionSelect, onShowMoreSearch, isDarkMode, isDesktop = false }: DashboardProps) {
  ...
  const handleDeleteRecentView = async (e: React.MouseEvent, viewId: number) => {
    e.stopPropagation(); // 아파트 클릭 이벤트 방지
                  ...
                  <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {rankingsData.rising.map((apt, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className={`flex-shrink-0 w-5 text-xs font-bold ${isDarkMode ? 'text-red-300' : 'text-red-600'}`}>
                              {index + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                  ...
                  <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {rankingsData.falling.map((apt, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className={`flex-shrink-0 w-5 text-xs font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                              {index + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                              <h4 className={`font-semibold text-xs truncate ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  ...
                  <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {rankingsData.rising.map((apt, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className={`flex-shrink-0 w-4 text-xs font-bold ${isDarkMode ? 'text-red-300' : 'text-red-600'}`}>
                              {index + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                              <h4 className={`font-semibold text-xs truncate ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  ...
                  <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {rankingsData.falling.map((apt, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className={`flex-shrink-0 w-4 text-xs font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                              {index + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                              <h4 className={`font-semibold text-xs truncate ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
